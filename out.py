...
class CommentDialog(wx.Dialog):
    ...
    def getListZhuti(self):
        ...
        sql_sel = "SELECT genre FROM av_genres WHERE type =1 ORDER BY num DESC"
        cur.execute(sql_sel)
        infos = cur.fetchall()
        print(infos)
        for info in infos:
            nameZhuti = info[0]
            self.listZhuti.append(nameZhuti)

    # 获取角色列表
    def getListJuese(self):
        def mdb_conn(password=""):
            # 功能：创建数据库连接 :param db_name: 数据库名称 :param db_name: 数据库密码，默认为空 :return: 返回数据库连接
        ...
        sql_sel = "SELECT genre FROM av_genres WHERE type =2 ORDER BY num DESC"
        cur.execute(sql_sel)
        infos = cur.fetchall()
        print(infos)
        for info in infos:
            nameJuese = info[0]
            self.listJuese.append(nameJuese)

    # 获取服装列表
    def getListFuzhuang(self):
        def mdb_conn(password=""):
            # 功能：创建数据库连接 :param db_name: 数据库名称 :param db_name: 数据库密码，默认为空 :return: 返回数据库连接
        ...
        sql_sel = "SELECT genre FROM av_genres WHERE type =3 ORDER BY num DESC"
        cur.execute(sql_sel)
        infos = cur.fetchall()
        print(infos)
        for info in infos:
            nameFuzhuang = info[0]
            self.listFuzhuang.append(nameFuzhuang)

    # 获取体型列表
    def getListTixing(self):
        def mdb_conn(password=""):
            # 功能：创建数据库连接 :param db_name: 数据库名称 :param db_name: 数据库密码，默认为空 :return: 返回数据库连接
        ...
        sql_sel = "SELECT genre FROM av_genres WHERE type =4 ORDER BY num DESC"
        cur.execute(sql_sel)
        infos = cur.fetchall()
        print(infos)
        for info in infos:
            nameTixing = info[0]
            self.listTixing.append(nameTixing)

    # 获取行为列表
    def getListXingwei(self):
        def mdb_conn(password=""):
            # 功能：创建数据库连接 :param db_name: 数据库名称 :param db_name: 数据库密码，默认为空 :return: 返回数据库连接
        ...
        sql_sel = "SELECT genre FROM av_genres WHERE type =5 ORDER BY num DESC"
        cur.execute(sql_sel)
        infos = cur.fetchall()
        print(infos)
        for info in infos:
            nameXingwei = info[0]
            self.listXingwei.append(nameXingwei)

    # 获取玩法列表
    def getListWanfa(self):
        def mdb_conn(password=""):
            # 功能：创建数据库连接 :param db_name: 数据库名称 :param db_name: 数据库密码，默认为空 :return: 返回数据库连接
        ...
        sql_sel = "SELECT genre FROM av_genres WHERE type =6 ORDER BY num DESC"
        cur.execute(sql_sel)
        infos = cur.fetchall()
        print(infos)
        for info in infos:
            nameWanfa = info[0]
            self.listWanfa.append(nameWanfa)

    # 获取其他列表
    def getListQita(self):
        def mdb_conn(password=""):
            # 功能：创建数据库连接 :param db_name: 数据库名称 :param db_name: 数据库密码，默认为空 :return: 返回数据库连接
        ...
        sql_sel = "SELECT genre FROM av_genres WHERE type =7 ORDER BY num DESC"
        cur.execute(sql_sel)
        infos = cur.fetchall()
        print(infos)
        for info in infos:
            nameQita = info[0]
            self.listQita.append(nameQita)
